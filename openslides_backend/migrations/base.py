...
class BaseMigrationSqlHelper(BaseSqlQueryHelper):
    ...
    def get_transfer_from_other_table_part(
        ...
        return sql.SQL("{columns_to_values} FROM {source_collection} AS s").format(
            ...
            source_collection=sql.Identifier(
    ...
    def get_lookup_part_from_other_table(
        ...
        return sql.SQL(
            "{table_t}.id = ANY(SELECT id FROM {table_m} WHERE {filter_string})"
        ).format(
            table_t=sql.Identifier(HelperGetNames.get_table_name(target_collection)),
...
class BaseMigration:
    ...
    def cleanup(self, curs: Cursor[DictRow]) -> None:
        ...
        Input:
            cursor
        """
        for table in self.copied_tables:
    ...
    def _get_filter_query_and_arguments(
        ...
        if isinstance(filter_or_condition, str):
            ...
            filter_string = BaseMigrationSqlHelper.build_filter_str(
                filter_or_condition, arguments, target_collection
            )
    ...
    def update_from_mig_table_sql(
        ...
        curs.execute(
            BaseMigrationSqlHelper.get_update_entries_sql(
                ...
                condition=condition
                or sql.SQL("{table}.id = s.id").format(table=sql.Identifier(table)),
            )
        )
    ...
    def update_array(
        ...
        condition: sql.Composable | None = (
            BaseMigration._get_filter_query_and_arguments(
                collection, filter_or_condition, filter_arguments, False
            )
