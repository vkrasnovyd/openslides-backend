...
class RemoveHelper:
    def handle_remove(
        ...
        if "collections" in remove:
            ...
            if isinstance(
                ...
                result += self.handle_remove_tree(remove_tree_dict, dc_remove_tree_dict)
        ...
        if "enum_types" in remove and isinstance(
            ...
            result += self.handle_remove_enum_types(
        ...
        if "_meta" in remove and isinstance(
            ...
            result += self.handle_remove_meta_attributes(
    ...
    def handle_remove_tree(
        ...
        for collection_name, collection_data in remove_tree_dict.items():
            for key, data in collection_data[1].items():
                match key:
                    case "fields":
                        result += self.handle_remove_fields(
                            data,
    ...
    def handle_remove_fields(
        ...
        if (
            ...
            result += self.handle_remove_table_fields(
                fields_to_remove,
                dc_fields_to_remove,
                collection_name,
            )
        if (
            ...
            result += self.handle_remove_field_attributes(
